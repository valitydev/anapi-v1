...
-type error() :: #{
    type        := error_type(),
    param_name  => swag_server:param_name(),
...
-type validation_opts() :: #{
    schema => swag_server_schema_validator:schema_validation_opts(),
...
validate(OperationID, Rule = schema, Name, Value, MsgType, ValidationOpts) ->
    case swag_server_schema_validator:validate(OperationID, Value, Name, MsgType, ValidationOpts) of
        ok ->
            ok;
        {error, Reason} ->
...
validate(OperationID, Rule, Name, Value, MsgType, ValidationOpts) ->
    case swag_server_param_validator:validate(OperationID, Rule, Name, Value, MsgType, ValidationOpts) of
        ok ->
            ok;
        Ok = {ok, _} ->
            Ok;
