...
-type param_base_rule() ::
    swag_server_common_validator:param_rule() |
    {enum, [atom()]} |
...
-type name()     :: swag_server:param_name().
...
validate_with_context(Rule, Value, Meta, ValidationOpts) ->
    case swag_server_custom_validator:validate_param(Rule, Value, Meta, ValidationOpts) of
...
do_validate({RuleName, _} = Rule, Value, _Meta, _Opts) when
    ...
    RuleName =:= format
->
    swag_server_common_validator:validate(Rule, Value).
