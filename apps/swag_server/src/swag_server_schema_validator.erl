...
-type msg_type() :: swag_server_validation:msg_type().
-type schema_validation_opts() :: #{msg_type() => validation_ruleset()}.
-type validation_opts() :: swag_server_validation:validation_opts().
...
-type context() ::
    #{
        ...
        msg_type     := msg_type(),
        definition_name     := swag_server:param_name(),
...
-spec validate(
    ...
    DefinitionName    :: swag_server:param_name(),
    MsgType    :: msg_type(),
...
validate(OperationID, Value, DefinitionName, MsgType, ValidationOpts) ->
    validate(OperationID, Value, DefinitionName, MsgType, swag_server_schema:get(), ValidationOpts).
...
validate(OperationID, Value, DefinitionName, MsgType, Schema, ValidationOpts) ->
    ...
    case jesse:validate_definition(swag_server_utils:to_list(DefinitionName), Schema, Value, Options) of
        {ok, _} ->
            ok;
        {error, [Error]} ->
...
maybe_use_custom_validator(Value, Attr, State) ->
    case swag_server_custom_validator:validate_schema(Attr, Value, State) of
        pass  ->
...
do_check_value(Value, {?FORMAT, Type}, State) when
    ...
    Type =:= <<"binary">>
->
    case swag_server_common_validator:validate({type, erlang:binary_to_atom(Type, utf8)}, Value) of
        error ->
...
do_check_value(Value, {?FORMAT, Format}, State) when
    ...
    Format =:= <<"uri">>
->
    case swag_server_common_validator:validate({format, erlang:binary_to_atom(Format, utf8)}, Value) of
        error ->
            jesse_error:handle_data_invalid(wrong_format, Value, State);
        _ ->
            State
